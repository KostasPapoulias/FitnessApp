...
// Pace plan sheet, opened before or during a run: a starting pace, a
// per-unit change, and the unit (km or minutes).

/** Pace options for this movement: 0.55×–2.2× its reference pace. */
const targetsFor = (referencePaceSec: number): number[] => {
  // Step size proportional to the pace, snapped to a readable grid
...
/** Seconds per unit the pace can change by. */
...
interface Props {
  ...
  /** Typical pace (s/km) for this movement; sets the wheel's range. */
  referencePaceSec: number
  /** Unit to open on: minutes for machines, kilometres for the road. */
...
export default function PaceSheet({
  ...
  const [startSec, setStartSec] = useState(() =>
    // Snapped to this movement's grid and range (the remembered plan may be from another activity)
  ...
  // The first four steps, so the athlete sees the plan (and where it hits the limit)
  const preview = [1, 2, 3, 4].map(step => targetForStep(draft, step))
  const flattens = deltaSec !== 0 && preview[3] === preview[2]

  return (
    // The shared sheet, like every other
    <BottomSheet
      title="Pace plan"
      subtitle={
    ...
    >
        ...
        <div className="mt-5 rounded-card border border-dark-600 bg-dark-700 px-4 py-3">
          ...
          <p className="text-[11px] text-dark-500 mt-2 leading-snug">
            {flattens
              // Said explicitly, since the clamped preview repeats a number
